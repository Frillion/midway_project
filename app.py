...
with urllib.request.urlopen('http://apis.is/petrol') as data:
...
def sub_site(company):
    if company in info['companies']:
        info['title'] = company
        return template('./views/sub.tpl',maininf=info,apidata=api,name=company)
    else:
        abort(404)
...
def sub_sub_site(company,location):
    if location in info['all_locations']:
        info['title'] = location
        return template('./views/sub_sub.tpl', maininf=info, apidata=api, name=location, company=company)
    else:
        abort(404)
